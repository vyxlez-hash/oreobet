...
export function CasinoGame({
  ...
  const play = async (extraChoice?: string) => {
    ...
    window.setTimeout(async () => {
      ...
      setShowResult(true);
      setBusy(false);
      startCooldown(gameId === 'roulette' ? 1.8 : 1.2);
            ...
            {busy
              ...
              : cooldown > 0
                ...
                : gameId === 'mines'
                  ? 'REVEAL TILE'
                  : 'PLAY ROUND'}
            {cooldown > 0 && <span className="cooldown-bar" key={`cooldown-${cooldown}`} />}
          </Button>
...
function RouletteVisual({
  ...
  return (
    <div className="relative z-10 flex flex-col items-center">
      <div
        className={cn(
          'roulette-stage game-round-enter',
