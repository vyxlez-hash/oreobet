...
export function DiceGame() {
  ...
  const canPlay = user && balance >= betAmount && betAmount > 0 && !rolling;
  ...
  const roll = async () => {
    ...
    const interval = setInterval(async () => {
      ...
      if (count > 12) {
        ...
        const combined = `${generateSeed()}:${clientSeed}:${nonceRef.current}`;
        const hash = hashSHA256(combined);
        const finalRoll = (parseInt(hash.slice(0, 8), 16) % 10000) / 100;
        const won = mode === 'under' ? finalRoll < target : finalRoll > target;
        ...
        if (betErr) {
          showToast('Failed to record bet');
          setRolling(false);
          return;
        }
        ...
        setLastResult({ roll: finalRoll, target, won, multiplier, payout, bet: betAmount });
        setHistory((prev) => [{ roll: finalRoll, target, won, multiplier, payout, bet: betAmount }, ...prev.slice(0, 9)]);
        setRolling(false);
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <Button
              onClick={roll}
              disabled={rolling || (!user || balance < betAmount)}
              className="w-full"
              size="lg"
            >
