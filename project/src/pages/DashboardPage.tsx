...
function AdminRequestsPanel() {
  ...
  const load=useCallback(async()=>{
    ...
    if(txRes.data){
      ...
      if(ids.length){const {data:ps}=await supabase.from('profiles').select('id,username').in('id',ids);const map:Record<string,string>={};(ps||[]).forEach((x:any)=>map[x.id]=x.username);setNames(map);}
    }
    const amap:Record<string,string>={};(addrRes.data||[]).forEach((x:any)=>amap[x.currency]=x.address);setAddresses(amap);setLoading(false);
  },[user]);
  useEffect(()=>{load()},[load]);
  if(!user?.isAdmin)return null;
  const saveAddress=async(currency:string)=>{setSaving(true);const {error}=await supabase.rpc('admin_update_crypto_address',{p_currency:currency,p_address:addresses[currency]||''});setSaving(false);if(error)showToast(error.message);else showToast(`${currency} deposit address updated.`)};
  const update=async(id:string,status:'completed'|'failed')=>{const note=window.prompt(`Admin note for ${status} (optional):`)||'';const {error}=await supabase.rpc('admin_update_transaction',{p_id:id,p_status:status,p_note:note});if(error)showToast(error.message);else{showToast('Request updated.');load();}};
  return <div className="space-y-6 mb-10">
    <Card className="p-6 border border-white/10"><div className="flex items-center justify-between mb-5"><div><div className="text-xs font-mono uppercase tracking-wider text-ink-400">Admin only</div><h2 className="text-xl font-bold text-white">Crypto deposit addresses</h2><p className="text-sm text-ink-400 mt-1">Only admins can change the addresses shown to users.</p></div><Shield className="w-6 h-6 text-white"/></div><div className="grid md:grid-cols-2 gap-4">{['BTC','SOL','LTC','ETH'].map(c=><div key={c} className="glass rounded-2xl p-4"><div className="text-xs font-mono text-ink-400 mb-2">{c}</div><div className="flex gap-2"><input value={addresses[c]||''} onChange={e=>setAddresses(a=>({...a,[c]:e.target.value}))} className="min-w-0 flex-1 bg-transparent text-white text-xs font-mono outline-none" placeholder={`${c} deposit address`}/><Button size="sm" disabled={saving} onClick={()=>saveAddress(c)}>Save</Button></div></div>)}</div></Card>
    <Card className="overflow-hidden"><div className="p-6 border-b border-white/5"><div className="text-xs font-mono uppercase tracking-wider text-ink-400">Admin only</div><h2 className="text-xl font-bold text-white mt-1">Deposit / Withdraw requests</h2><p className="text-sm text-ink-400 mt-1">See who requested it, amount, currency, wallet/hash and approve or fail it.</p></div>{loading?<div className="p-10 text-center text-ink-400">Loading requests...</div>:requests.length===0?<div className="p-10 text-center text-ink-400">No requests.</div>:<div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-xs font-mono uppercase text-ink-400 border-b border-white/5"><th className="p-4">User</th><th className="p-4">Type</th><th className="p-4">Amount</th><th className="p-4">Coin</th><th className="p-4">Hash / Wallet</th><th className="p-4">Status</th><th className="p-4">Action</th></tr></thead><tbody>{requests.map(r=><tr key={r.id} className="border-b border-white/[.03] align-top"><td className="p-4 text-white">@{names[r.user_id]||r.user_id.slice(0,8)}</td><td className="p-4 uppercase text-ink-200">{r.type}</td><td className="p-4 font-mono text-white">{formatUSD(Number(r.amount))}</td><td className="p-4 text-ink-200">{r.currency}</td><td className="p-4 max-w-[260px]"><code className="text-[10px] text-ink-300 break-all">{r.type==='deposit'?(r.tx_hash||'No hash supplied'):(r.wallet_address||'No wallet')}</code></td><td className="p-4"><span className="text-xs font-mono uppercase text-ink-200">{r.status}</span></td><td className="p-4">{r.status==='pending'&&<div className="flex gap-2"><Button size="sm" onClick={()=>update(r.id,'completed')}>Approve</Button><Button size="sm" variant="outline" onClick={()=>update(r.id,'failed')}>Fail</Button></div>}{r.admin_note&&<div className="text-[10px] text-ink-500 mt-2">{r.admin_note}</div>}</td></tr>)}</tbody></table></div>}</Card>
  </div>;
}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl overflow-hidden border border-white/10 bg-white/5 flex items-center justify-center shrink-0 shadow-lg">
            <GeneratedAvatar username={user.username} size={80} />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-2"><p className="text-ink-300">Level {user.level} · Member since {new Date(user.joined).toLocaleDateString()}</p><span className="text-[10px] font-mono uppercase rounded-full border border-white/10 px-2 py-1 text-ink-300">{user.isAdmin ? 'Admin' : user.role}</span></div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 animate-fade-in">
          <Card className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-bold text-white">Profile avatar</h3>
            </div>
            <div className="flex items-center gap-5">
              <GeneratedAvatar username={user.username} size={80} />
              <div>
                <p className="text-sm text-ink-200 mb-2">Your avatar is generated automatically from your username.</p>
                <p className="text-xs text-ink-400">Change your username to generate a different avatar.</p>
              </div>
            </div>
          </Card>
