...
export function Navbar() {
  ...
  const visibleNavItems = user ? navItems.filter((item) => item.id !== 'home') : navItems;
      ...
      >
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            <button onClick={() => handleNav(user ? 'games' : 'home')} className="flex items-center gap-2.5 group">
              ...
              {user ? (
                <>
                  <button
                    ...
                    title={`@${user.username}`}
                  >
                    <span className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center border border-white/10 overflow-hidden">
                      <GeneratedAvatar username={user.username} size={28} />
                    </span>
                    <span className="text-sm text-white max-w-28 truncate">@{user.username}</span>
                    {user.isAdmin && (
                      <span className="text-[10px] font-mono uppercase tracking-wider rounded-md border border-white/20 px-1.5 py-0.5 text-white">Admin</span>
                    )}
                  ...
                  <>
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className="w-7 h-7 rounded-full bg-white/10 border border-white/10 overflow-hidden flex items-center justify-center">
                          <GeneratedAvatar username={user.username} size={28} />
                        </span>
